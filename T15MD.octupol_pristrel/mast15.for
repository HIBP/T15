      ...
      subroutine ORCPASS15(xlast,ylast,zlast,vxlast,vylast,vzlast,dt1,
     *              dt_small_primary,dt_small_secondary,isOK)
c ¤Єр яЁюуЁрььр яюярфрхЄ т ЎхэЄЁ ярЄЁєсър ш тючтЁр∙рхЄ ъююЁфшэрЄ√
c яюёыхфэхщ Єюўъш ЄЁрхъЄюЁшш ш ёъюЁюёЄш
	use opengl
      use msfwin
...
C*****************************************************************
 ...
 1550 CONTINUE
       ...
       beta0= beta_ext
       zi_entered= zi_ext !1.25-2.5*beta0
       beta0step=0.
       dt1=dt_ext
...
c         if (ision.eq.0) then
             print *, 'plate3'
             goto 812 !чряЁхЄшЄ№ яхЁхёхърЄ№ ёЄхэъє
c	   else
c             print*, 'bl2a'
c             isbadluck=1
c	       goto 1999
...
c      close(77)
 ...
 589  CONTINUE
	...
	*                               .and.intersect.eq.1) then
      ...
      print *, 'Work complete...' 
      if (isOK.eq.1) then
      open (1,FILE='grid.dat',STATUS='UNKNOWN',ACCESS='APPEND')
      write (1,799) E,alfa*degrad, 
     *    (xion2+xion1)/2.,(yion2+yion1)/2.,(zion2+zion1)/2.,intersect
 799  FORMAT (5F15.5,3X,i1)
	close(1)
      open (1,FILE='zones.dat',STATUS='UNKNOWN',ACCESS='APPEND')
	...
	zg(NG)=(zion2+zion1)/2.
      ...
      open (1,FILE='griddata.dat',ACCESS='APPEND')
