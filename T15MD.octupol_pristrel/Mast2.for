      ...
      subroutine ORCPASS2(Uneeded
     *,xbeg,ybeg,zbeg,vxbeg,vybeg,vzbeg
     *,dt1,dt_small_primary,dt_small_secondary,isOK)
c программа подбирает напр. на октуполе дл€ попадани€ в xdconst,ydconst
c
	use opengl
      use msfwin
...
C idflag=0 •бЂ® ≠• ≠г¶≠† Ѓѓв®ђ®І†ж®п ѓЃ beta §Ђп intersect=1
       idflag=0
C*****FILE FIELD.DAT (FIELD NETWORK) EXISTANSE PARAMETER*****
      IFOP=0
c      ifMntFldOpnd=0

C ÷ентр пол€рной системы координат
      Xnc=-1160.
	Ync= 30.
...
C****     work now ***********************
         smeshenie00= 0. !-0.75 !0.7
         smeshenie0= 0. !-0.75 !0.7
        alfarazl0=0. !-0.36*(3.14159265/180.)
        alfarazl00=0. !-0.36*(3.14159265/180.)
        farang=58.75*3.14159265/180.
 ...
 5386 CONTINUE
      ...
      if (isBeamLoss.eq.0) then
	   ...
	   NBeam=2
         ...
         NBegin=1 !2
c надо вернутьс€ к 2-ке, если выше на 7 строчек стоит isBeamLoss=0
	   NBeam=10
	endif
...
c      U2blp=0. !начальное приближение
	Ustep= 0.5
	iplus=0
	iminus=0
 196  CONTINUE
         ...
         I=1
          iCounter=0
c iCT.eq.0 вЃЂм™Ѓ ѓа® ѓ•аҐЃђ ѓаЃеЃ§•
          iCT=0
...
c The flag to alert port beam loss
	  ...
	  zoldgr=znewgr
          ...
          A1=2.*A	!сразу вторична€
	    qm=2.*qmTl !сразу вторична€
          VXIT1=DU1(4)
          VYIT1=DU1(5)
          VZIT1=DU1(6)
...
c	   bline=yd-cline*xd
	   ...
	   ypo=DU1(2)
         ...
         IF(xpo.GE.(xd0-0.001))  GOTO 129
          if(imkI.eq.0) GOTO 812
           DT2=DT2/2
           EPSO=0.5*EPSO
c           print*, 'imki=1',dt2,r2,rdno
            GOTO 812

 129       IF(xpo.LE.(xd0+0.001)) GOTO 3
            imkI=1
            DT2=DT2/2
            EPSO=0.5*EPSO
c           print*, 'perelyot',DT2,r2,rdno
            i=i-1
            DU1(1)=XIT1(IP,i)
            DU1(2)=YIT1(IP,i)
            DU1(3)=ZIT1(IP,i)
    ...
    3    EPSO=EPS1
        ...
        Vxpo=DU1(4)
	  Vypo=DU1(5)
c        angpo=atan_new(Vypo,Vxpo)
        if (Vxpo.eq.0) then
	    print *, 'angpo.eq.0'
          angpo=0.
	  else
	    angpo=atan(Vypo/Vxpo)
	  endif
	  anggoal=(120.+analangle)*(3.14159265/180.)
        if (abs(ustep).lt.1.E-6) goto 342
        if (abs(U2blp).gt.30.) then
	     isOK=0
	     goto 342
	  endif


        if (ypo.gt.yd0+0.001) then
cc        if (angpo.gt.anggoal+0.0001) then
	     U2blp=U2blp+Ustep
	     iplus=1
           print *, 'U2blp: ',U2blp
	     print *, 'y:',ypo,yd0
cc	     print *, 'ang:',angpo*180./3.14159265,anggoal*180./3.14159265
           print *, 'Vel:',Vxpo,Vypo
	     if (iminus.eq.1) then
	       Ustep=Ustep/2.
           endif
c	     read(*,*)
        ...
        if (ypo.lt.yd0-0.001) then
cc        if (angpo.lt.anggoal-0.0001) then
	     U2blp=U2blp-Ustep
	     iminus=1
           print *, 'U2blp: ',U2blp
	     print *, 'y:',ypo,yd0
