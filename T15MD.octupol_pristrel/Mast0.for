...
C x<>y
          ...
          xresult=(yd-yit1(ip,i-1))*(xit1(ip,i)-xit1(ip,i-1))/
     *            (yit1(ip,i)-yit1(ip,i-1)) + xit1(ip,i-1)
          print *, 'x:',xresult, xd
...
c       print*, 'Кричи: "Ура!"'
 ...
 8739   CONTINUE
           ...
           print *, 'x=',xresult,xd,' U: ',Ufar
        if (xresult.lt.xd-0.0001) then
           iplus=1
	     if (iminus.eq.1) Ustep=Ustep/2.
	     Ufar=Ufar-Ustep
c           print *, 'y=',yresult,yd,' U: ',Ufar,iminus,iplus
c		   read (*,*)
	     goto 193
	  endif
        if (xresult.gt.xd+0.0001) then
           iminus=1
