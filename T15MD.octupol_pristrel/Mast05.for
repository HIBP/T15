      ...
      subroutine ORCPASS05(isOk)
c ¤Єр яЁюуЁрььр яюярфрхЄ т Єюўъє яЁшёЄЁхыъш фы  beta трЁ№шЁє  Ubf1
	use opengl
      use msfwin

   
      COMMON H,A1,RTOK,RPL,IPO,IVIT,IOH,
     *XIT1(1,700),XIT2(1,2000),YIT1(1,900),YIT2(1,2000),ZIT1(1,1000),
     *ZIT2(1,2000),XIT3(1,700),YIT3(1,700),ZIT3(1,700),
...
C^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	*                    ,Ubeta1_1,Ubeta1_2
     ...
     *DU2(6),LT(6),P(7),EE(21),ZDEF(21),X13(100),Y13(100),
     *Z13(100),IV(10),JV(10),JAXV(10),JA1V(10),XITI(500),
     *YITI(500),ZITI(500),XITJ(500),YITJ(500),ZITJ(500),
c     *ZIT1(1,1000),ZIT2(1,2000),
     *rtpp(4),dalf(4),
...
c        zi=zi_entered
      ...
      SNAME1='output\TRAJ0000.DAT'
      WRITE(SNAME1(12:15),'(1I4)') kz
      WRITE(*,10) SNAME1     
...
C              ZD=-15.
               apertura=0.
               sdvigdet=0.
               detangle=25.*(3.14159265/180.)
               xd0=xaim + sdvigdet*sin(detangle)			  !xUcf
               yd0=yaim  - sdvigdet*cos(detangle) !+ greshki	yUcf
...
C                    INITIAL POINT OF TRAJECTORY
      ...
      CALL PATRUBOK()
	 CALL AXES (xmi,ymi,xma,yma,10.,10.)
...
c         endif
          ...
          DT2=DT1
          EPSO=EPS1
          print*, 'Uplate2=',uplate2,' E=',E,'DT1=',DT1
          GOTO 1
C&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
  812   CONTINUE
         fi_old=fi_new
...
c	read(*,*)
 ...
 4125   CONTINUE
        if ((rt1.gt.rdno.and.DU1(2).lt.45.).or.DU1(2).lt.-45.) then
          isOK=0
          print*, '2'
          goto 9999
        endif
...
C*******************************************************************
  ...
  81      I=I+1
          ...
          YIT1(IP,I)=DU1(2)
          ZIT1(IP,I)=DU1(3)
          if (xit1(ip,i).gt.max(xd+25.,xi+5.).and.ision.eq.0.) then
             isOK=0
 		   goto 9999
	    endif
...
c          hzapz1(i)=hz/1.E4
          ...
          if(rt1.ge.(rpl+zapas).and.ision.eq.0.) goto 1
          if (yit1(ip,i).gt.y_border) goto 1
...
c      if (isNoField.eq.1) then

c		call save_trajectory(ip,I,J)
c		kz=kz-1
...
c          hzapz2(j)=hz/1.E4
	...
	if ((cc*xit2(ip,j-1)+bb-yaim)*(cc*xit2(ip,j)+bb-yaim).le.0.) then
	   yy2=cc*xit2(ip,j)+bb
         yy1=cc*xit2(ip,j-1)+bb
         if (yy2.eq.yy1) then
	      z_current=zit2(ip,j)
	   else
            z_current=zit2(ip,j-1)-(zit2(ip,j-1)-zit2(ip,j))/
     *             (Yy1-Yy2)*(Yy1-yaim)
	   endif
c         print*, zit2(ip,j-1),zit2(ip,j),z_current
c         read(*,*)
	endif
...
c        if (xit2(ip,j).gt.31..and.xit2(ip,j-1).lt.31.
c     *   .and. abs(yit2(ip,j)).gt.4.) goto 812
c       if (xit2(ip,j).lt.xit2(ip,1)) goto 812
...
c  Important to rem out
        if (j.gt.20) then
           if ((xit2(ip,j).lt.xit2(ip,j-1)
	*     .and.yit2(ip,j).gt.yit2(ip,j-1))
	*     .and.ision.eq.0) then
           print *,'curl'
	     imka=0
	     imko=0
...
c	endif
 ...
 6398    CONTINUE
cc      OPEN(55,FILE=SNAME1,STATUS='UNKNOWN')
c         write(55,*) NN1,I+1,NN2+1,J+I
c         open (55, FILE='traj.dat', STATUS='UNKNOWN')
cc      write (55,*) 'E=',E,' alfa=',alfa*degrad,' PrimaryPoints=',I
cc      do kk=1,I
cc       write(55,*) "(",xit1(ip,kk)*10,yit1(ip,kk)*10,zit1(ip,kk)*10,")"
c       write(55,*) xit1(ip,kk),yit1(ip,kk),zit1(ip,kk)
cc      enddo
cc	do kk=1,J
cc       write(55,*) "(",xit2(ip,kk)*10,yit2(ip,kk)*10,zit2(ip,kk)*10,")"
c       write(55,*) xit2(ip,kk),yit2(ip,kk),zit2(ip,kk)
cc      enddo
cc      close (55)
...
c     if (fino.gt.fiold1) print *, 'DANGER'
      if ((ind.eq.1).and.(fino.lt.fdl)) goto 1998
      if (fino.lt.fdl) then 
c      print *, 'In analyser'
      ind=1
      xion1=xit1(ip,i)
...
c      kz=kz-1
 ...
 1998 CONTINUE
      ...
      yzon(icount)=yit1(ip,I)
      zzon(icount)=zit1(ip,I) 
c      print*, 'Computing ionZone: ',icount
      if ((fino.lt.fdp).or.
     *((fino.gt.fiold1).and.(fiold1.gt.fiold2))) then
           icount=icount-1
           goto 1999
      endif
c     print*, 'Считаем ЗИ: ',icount
      if (fino.lt.fiold1) then
      xion2=xit1(ip,i)
      yion2=yit1(ip,i)
      zion2=zit1(ip,i)
      else
...
c      close(77)
 ...
 589  CONTINUE
	...
	flam=sqrt((xion1-xion2)**2+(yion1-yion2)**2)
      if (((flam.le.0.1.and.icount.le.10).or.flam.ge.5.)
	*                               .and.intersect.eq.1) then
         isOK=-1
      endif
...
c     *    (xion2+xion1)/2.,(yion2+yion1)/2.,(zion2+zion1)/2.,intersect
 799  FORMAT (5F15.5,3X,i1)
c	close(1)
c      open (1,FILE='zones.dat',STATUS='UNKNOWN',ACCESS='APPEND')
c      write (1,*) '(',(xion1+RTOK)*10.,yion1*10.,zion1*10.,')'
c      write (1,*) '(',(xion2+RTOK)*10.,yion2*10.,zion2*10.,')'
c	close(1)
      NG=NG+1
c      xg(NG)=(xion2+xion1)/2.
c	yg(NG)=(yion2+yion1)/2.
c	zg(NG)=(zion2+zion1)/2.
c      inter(NG)=intersect
c      Ebeam(NG)=E
c      ugolAlfa(NG)=Alfa*degrad
...
c        idflag=0
c        isplates=0
c           beta0=beta0*degrad
           alfa0=alfa0*degrad
c             z_current=DU2(3)

	print *, 'z:', z_current, zgoal, 'Ubf1:' ,Ubf1
...
c	read (*,*)
	...
	if (z_current.gt.zgoal+eps_z) then
         iplus=1
	   if (iminus.eq.1) Ubetastep=Ubetastep/2.
	   Ubf1=Ubf1+Ubetastep
	   goto 4972

	   call save_trajectory(ip,I,J)
	   kz=kz-1

	endif
	if (z_current.lt.zgoal-eps_z) then
         iminus=1
	   if (iplus.eq.1) Ubetastep=Ubetastep/2.
	   Ubf1=Ubf1-Ubetastep
	   goto 4972
	endif
c	else
...
c	   goto 4972
	...
	SUBROUTINE save_trajectory(ip,I,J)
	common /kz/ kz
      COMMON H,A1,RTOK,RPL,IPO,IVIT,IOH,
     *XIT1(1,700),XIT2(1,2000),YIT1(1,900),YIT2(1,2000),ZIT1(1,1000),
     *ZIT2(1,2000),XIT3(1,700),YIT3(1,700),ZIT3(1,700)
	...
	*                    ,Ubeta1_1,Ubeta1_2 
      ...
      SNAME1='output\TRAJ0000.DAT'
	WRITE(SNAME1(12:15),'(1I4)') kz
      OPEN(55,FILE=SNAME1,STATUS='UNKNOWN')
	
      write(55,*) H0, CurTot, E, Uplate2

	do kk=1,I
c		write(55,*) "(",xit1(ip,kk)*10,yit1(ip,kk)*10,zit1(ip,kk)*10,")"
		write(55,*) xit1(ip,kk)*10,yit1(ip,kk)*10,zit1(ip,kk)*10, 1
	enddo

 	do kk=1,J
c		write(55,*) "(",xit2(ip,kk)*10,yit2(ip,kk)*10,zit2(ip,kk)*10,")"
		write(55,*) xit2(ip,kk)*10,yit2(ip,kk)*10,zit2(ip,kk)*10, 2
