...
C              ZD=-15.
               ...
               xd0=xdet + sdvigdet*sin(detangle)
               yd0=ydet  - sdvigdet*cos(detangle) !+ greshki
...
c      if (isintersect(x_pl_12,y_pl_12,x_pl_22,y_pl_22,
c     *       xit1(ip,i-1),yit1(ip,i-1),xit1(ip,i),yit1(ip,i)).eq.2) then
c	       print *, xit1(ip,i),yit1(ip,i)
c	       print *, x_pl_12,y_pl_12
c	       print *, x_pl_22,y_pl_22
c	       print *, 'port1'
c          isOK=0
c             goto 9999 !чряЁхЄшЄ№ эх яЁюыхЄрЄ№ т яюЁЄ
c	endif
...
c проверка не забрались ли мы слишком высоко
        if ((yit2(ip,j).gt.(yd+90.)).and.(isan.eq.0.)) goto 812
...
c      ╩ююЁфшэрЄ√ ёЄхэюъ
	 ...
	 xpoB2=111.28 !62.406
	 ypob2=-24.24 !-90.231
	if (isintersect(xpoA1,YpoA1,xpoB1,ypoB1,
     *       xit2(ip,j-1),yit2(ip,j-1),xit2(ip,j),yit2(ip,j)).eq.1
     *       .and.ision.eq.0) then
c         if (ision.eq.0) then
             print *, 'plate3'
cc             goto 812 !чряЁхЄшЄ№ яхЁхёхърЄ№ ёЄхэъє
c	   else
c             print*, 'bl2a'
c             isbadluck=1
c	       goto 1999
c	   endif
      ...
      if (isintersect(xpoA2,YpoA2,xpoB2,ypoB2,
     *       xit2(ip,j-1),yit2(ip,j-1),xit2(ip,j),yit2(ip,j)).eq.1
     *       .and.ision.eq.0) then
c         if (ision.eq.0) then
             print *, 'plate2'
...
c     *      xit2(ip,j).gt.xpse(nnetr)) then
         if (xit2(ip,j).gt.xd+25..or.yit2(ip,j).lt.yd-100.
	*      .or. yit1(ip,i).lt.yd-100.) then
            isOK=0
	        print*, '3'
            goto 9999 !515
	  endif
...
C    *      .and.(imka.eq.0)) goto 9999
        ...
        if (iCT.eq.0) then
            iCT=1
c           if (fino.lt.fdl) then
            if (yit2(ip,j).lt.(yd+0.01)) then
              intersect=1
	        print *, 'intersect=1'
...
c      close(77)
 ...
 589  CONTINUE
	...
	*                               .and.intersect.eq.1) then
	...
	open (1,FILE='output\grid.dat',STATUS='UNKNOWN',ACCESS='APPEND')
      write (1,799) E,Uplate2-Uplate1, 
     *    (xion2+xion1)/2.,(yion2+yion1)/2.,(zion2+zion1)/2.,intersect
 799  FORMAT (5F15.5,3X,i1)
	close(1)
      
	open (1,FILE='output\zones.dat',STATUS='UNKNOWN',ACCESS='APPEND')
      write (1,*) '(',xion1*10.,yion1*10.,zion1*10.,')'
      write (1,*) '(',xion2*10.,yion2*10.,zion2*10.,')'
	close(1)
	...
	zg(NG)=(zion2+zion1)/2.
      inter(NG)=intersect
      Ebeam(NG)=E
      ugolAlfa(NG)=Uplate2-Uplate1
      
	open (1,FILE='output\griddata.dat',ACCESS='APPEND')
	write(1,769) xg(Ng),yg(Ng),zg(NG),Ebeam(Ng),ugolalfa(Ng),inter(NG)
      close(1)
...
c       print*, 'Кричи: "Ура!"'
      ...
      enddo
        ...
        function atan_new(y,x)

        pi=3.14159265

        if (abs(y).lt.1.E-5) then
           if (x.ge.0) then
              atan_new=0.
           endif
           if (x.lt.0) then
        ...
        if ((y.gt.0.).and.(x.lt.0)) then
           atan_new=atan(y/x) +  pi
        endif

        if ((y.lt.0.).and.(x.lt.0)) then
           atan_new=atan(y/x) + pi
        endif

        if ((y.lt.0.).and.(x.gt.0)) then
           atan_new=atan(y/x) + 2.*pi
        endif
      ...
      function isintersect(xa,ya,xb,yb,x1,y1,x2,y2)
C ЇєэъЎш  яЁютхЁ хЄ, яхЁхёхър■Єё  ыш юЄЁхчъш AB ш 12
      common /patrcoeff/ patrcoeff
	common /zcoeff/ z_ini_1,z_ini_2,zcoeff,z_res


      if (xa.eq.xb) then
        print *, 'Decision might be not accurate ...'
        xb=xa+1.E-8
	  return
	endif
...
c яЁ ьр  12
	...
	b12=y1-c12*x1
      ...
      if (cab.eq.c12) then ! хёыш яЁ ь√х ярЁрыыхы№э√,
        isintersect=0	     ! Єю юэш эх яхЁхёхър■Єё  :)
	  return
	endif

c Єюўър яхЁхёхўхэш  яЁ ь√ї
	xp=(bab-b12)/(c12-cab)
	yp=c12*xp+b12

	if ((y1-yp)*(yp-y2).gt.0.) then
	  isintersect=2
          patrcoeff=(xp-xa)/(xb-xa)
	    z_res=z_ini_1+patrcoeff*(z_ini_2-z_ini_1)
          zz1=-2.5
	    zz2= 2.5
	    zcoeff=(z_res-zz1)/(zz2-zz1)
        if ((ya-yp)*(yp-yb).gt.0.) then
          isintersect=1
	  endif
	else
        isintersect=0
...
c	if ((y1-yp)*(yp-y2).gt.0.and.(ya-yp)*(yp-yb).gt.0.) then
c          isintersect=1
c          patrcoeff=(xp-xa)/(xb-xa)
c	elseif(((y1-yp)*(yp-y2).gt.0.and.(ya-yp)*(yp-yb).le.0.).or.
c	*       ((y1-yp)*(yp-y2).le.0.and.(ya-yp)*(yp-yb).gt.0.)) then
c		isintersect=2
c	else
c          isintersect=0
c	endif
